...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  // Try to load assets, fallback to placeholder if not available
  let nodes: any = null;
  ...
  try {
    texture = useTexture('/assets/lanyard/lanyard.png') as THREE.Texture;
  } catch (error) {
    ...
    if (fixed.current && card.current && j1.current && j2.current && j3.current) {
      [j1, j2].forEach(ref => {
        if (ref.current) {
          ...
          const lerped = (ref.current as any).lerped;
  ...
  // If assets aren't loaded, show a placeholder
  if (!nodes || !materials) {
    return (
      <group position={[0, 4, 0] as [number, number, number]}>
        <RigidBody ref={fixed} {...segmentProps} type="fixed" />
        <RigidBody position={[0.5, 0, 0] as [number, number, number]} ref={j1} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[1, 0, 0] as [number, number, number]} ref={j2} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[1.5, 0, 0] as [number, number, number]} ref={j3} {...segmentProps}>
          <BallCollider args={[0.1]} />
        </RigidBody>
        <RigidBody position={[2, 0, 0] as [number, number, number]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
